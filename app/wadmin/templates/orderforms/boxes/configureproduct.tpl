...
<tr class="orderrow1"><td>{$LANG.orderproduct}</td><td><strong>{$productinfo.groupname} - {$productinfo.name}</strong></td></tr>
<tr class="orderrow2"><td>{$LANG.orderdesc}</td><td>{$productinfo.description}</td></tr>
<tr class="orderrow1"><td>{$LANG.orderbillingcycle}</td><td><input type="hidden" name="previousbillingcycle" value="{$billingcycle}" />{if $pricing.type eq "free"}
...
<tr class="orderrow{if $num % 2}2{else}1{/if}"><td>{$configoption.optionname}:</td><td>
...
<tr class="orderrow{if $num % 2}2{else}1{/if}"><td>{$customfield.name}</td><td>{$customfield.input} {$customfield.description}</td></tr>
{/foreach}
<tr class="orderheadingrow"><td colspan="2"></td></tr>
<tr><td height="10"></td></tr>
{/if}
...
<tr class="orderrow{if $num % 2}2{else}1{/if}"><td>{$LANG.orderdomain} {$num+1}</td><td>{$domain.domain}{if $domain.regperiod} ({$domain.regperiod} {$LANG.orderyears}){/if}</td></tr>
{/foreach}
{/if}
{if $additionaldomainfields}
{foreach key=domainfieldname item=domainfield from=$additionaldomainfields}
<tr class="orderrow{if $num % 2}2{else}1{/if}"><td>{$domainfieldname}</td><td>{$domainfield}</td></tr>
{/foreach}
{/if}
<tr class="orderheadingrow"><td colspan="2"></td></tr>
<tr><td height="10"></td></tr>
...
<p align="center">{if $editconfig}<input type="submit" value="{$LANG.updatecart}" class="buttongo" />{else}<input type="submit" value="{$LANG.addtocart}" class="buttongo" />{/if}</p>
