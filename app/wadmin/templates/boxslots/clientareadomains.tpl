<h3>{$LANG.clientareanavdomains}</h3>
...
<div class="row panel radius">
    ...
    <div class="large-6 columns">{$numproducts} {$LANG.recordsfound},  {$LANG.page} {$pagenumber} {$LANG.pageof} {$totalpages}</div>
    
     
      <div class="large-4 columns">

        <input type="text" name="q" value="{if $q}{$q}{else}{$LANG.searchenterdomain}{/if}" class="imput-domains" onfocus="if(this.value=='{$LANG.searchenterdomain}')this.value=''" />
       </div> 
       <div class="large-2 columns"> 
        <input type="submit" value="{$LANG.searchfilter}" class="button radius postfix" />
      </div>
