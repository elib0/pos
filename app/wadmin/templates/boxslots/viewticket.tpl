...
{if $showclosebutton}
<p align="center">
  <input type="button" class="tiny button  radius" value="{$LANG.supportticketsstatuscloseticket}" onclick="window.location='{$smarty.server.PHP_SELF}?tid={$tid}&amp;c={$c}&amp;closeticket=true'" />
</p>
{/if}
...
<form method="post" action="{$smarty.server.PHP_SELF}?tid={$tid}&amp;c={$c}&amp;postreply=true" enctype="multipart/form-data">
  ...
  <p align="center">
    <input type="submit" value="{$LANG.supportticketsticketsubmit}" class="tiny button  radius" />
  </p>
</form>
{/if}<br />