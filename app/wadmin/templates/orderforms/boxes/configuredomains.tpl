...
{if $domain.eppenabled}<tr class="orderrow1"><td>{$LANG.domaineppcode}</td><td><input type="text" name="epp[{$num}]" size="20" value="{$domain.eppvalue}" /> {$LANG.domaineppcodedesc}</td></tr>{/if}
{if $domain.dnsmanagement}<tr class="orderrow2"><td>{$LANG.domaindnsmanagement}</td><td><input type="checkbox" name="dnsmanagement[{$num}]"{if $domain.dnsmanagementselected} checked{/if} /> {$domain.dnsmanagementprice}</td></tr>{/if}
{if $domain.emailforwarding}<tr class="orderrow1"><td>{$LANG.domainemailforwarding}</td><td><input type="checkbox" name="emailforwarding[{$num}]"{if $domain.emailforwardingselected} checked{/if} /> {$domain.emailforwardingprice}</td></tr>{/if}
{if $domain.idprotection}<tr class="orderrow2"><td>{$LANG.domainidprotection}</td><td><input type="checkbox" name="idprotection[{$num}]"{if $domain.idprotectionselected} checked{/if} /> {$domain.idprotectionprice}</td></tr>{/if}
{foreach key=domainfieldname item=domainfield from=$domain.fields}
<tr class="orderrow1"><td>{$domainfieldname}:</td><td>{$domainfield}</td></tr>
{/foreach}
{/if}
<tr class="orderheadingrow"><td colspan="2"></td></tr>
