...
<tr class="{cycle values="orderrow1,orderrow2"}"><td width="10"><input type="radio" name="pid" value="{$product.pid}"{if $product.qty eq "0"} disabled{/if} /></td><td><strong>{$product.name}</strong> {if $product.qty!=""}<em>({$product.qty} {$LANG.orderavailable})</em>{/if} - {$product.description}</td></tr>
{/foreach}
<tr class="orderheadingrow"><td colspan="2"></td></tr>
</table>
